 ...
 *
 * @details
 * Fix interval parameter controls the time between GNSS receiver starts.
 * It also controls whether the GNSS receiver is stopped after a valid PVT
 * estimate. The fix interval parameter determines the mode of navigation.
 ...
 * In this mode once a valid PVT estimate is produced, the GNSS receiver is
 * turned off indefinitely. It will not resume navigation without explicit
 * actions by the application processor. To do another single-fix, the
 * application processor must first stop the GNSS module  and then start the
 * GNSS module again.
 *
 * Continuous navigation mode is engaged by setting fix interval to 1. In this
 * mode the GNSS receiver continues producing fixes at 1 Hz rate without any
 * time limit.
 *
 * Periodic navigation mode is engaged by setting the fix interval to value
 * other than zero or one. In this mode the GNSS receiver is turned off after
 * each valid PVT estimate, and turned back on periodically after each fix
 * interval has passed.
 */
 ...
 *
 * The GNSS receiver may track signals (i.e. run) either continuously or in duty
 * cycles. In continuous tracking the received GNSS signal has no gaps. Best
 * performance is achieved with continuous tracking. In duty-cycled tracking the
 * GNSS receiver operates in on-and-off cycles consisting of tracking and
 * sleeping. The received GNSS signal has therefore short gaps during the sleep
 * periods. Obviously, duty-cycled tracking reduces the power consumption at the
 * expense of performance.
 * 
 * Duty cycle denotes the fraction of one cycle in which the GNSS receiver is
 * tracking. In duty-cycled tracking the tracking period is fixed to 100 ms,
 * and the sleeping period is fixed to 400 ms. Consequently, the total time
 * needed to complete an on-and-off cycle is 500 ms. These values give 20% duty
 * cycle.
 * 
 * Since the GPS data bit length is 20 ms, and the smallest meaningful data
 * primitive is 30-bit word (i.e. 600 ms), it is impossible to decode any data
 * during duty-cycled tracking. This has significant impact on the performance.
 * It means that any new GNSS signal cannot be utilized before the continuous
 * tracking is resumed to enable the decoding of the navigation data, or AGPS is
 * used to provide relevant navigation data.
 *
